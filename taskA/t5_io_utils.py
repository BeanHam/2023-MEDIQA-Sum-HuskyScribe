...
from model_constants import topic_ontology, SECTION2FULL
def convert_dialogue_chunks2t5input_topic(dialogue_chunks):
...
def convert_dialogue_chunks2t5input_seq(dialogue_chunks):
    ...
    for tid, turn in enumerate(dialogue_chunks):
        ...
        t5_input = f"previous section:  {x['sentence_AA']} {x['sentence_A']}\ncurrent section: {x['sentence_B']}\nnext section: {x['sentence_BB']} {x['sentence_BBB']} Question: what is the current section topic ? and Is the current section a different topic from the previous section? {topic_ontology} " 
...
def write_dialogue_chunk_list(list_of_dialogue_chunks, output_file):
    '''
    Overview
    --------
    ...
    for dialogue_chunks in list_of_dialogue_chunks:
        ...
        t5_data_t  = convert_dialogue_chunks2t5input_seq(dialogue_chunks)
